...
df = pd.read_csv(f"/home/mohit/Documents/project/EmbedTAD/data/resutls/comparison/gm12878_adj_sqr.csv")
...
summary.to_csv('gm12878_adj_r_statistics.csv', index=False)
...
plt.savefig("gm12878_adj_square_boxplot.png", dpi=300, bbox_inches="tight")
...
for resolution in smooth_df["Resolution"].unique():
    ...
    g.map_dataframe(sns.lineplot, x="X_Axis", y="Value")
    
    # Add labels, titles, and adjust layout
    g.set_axis_labels("Distanve (Kb)", f"TADadjR$^2$")
    ...
    plt.savefig(f"gm12878_{resolution}_adj_r_lineplots.png", dpi=300, bbox_inches="tight")


df = pd.read_csv(f"/home/mohit/Documents/project/EmbedTAD/data/resutls/comparison/ch12lx_adj_sqr.csv")
# df = df[df["Resolution"] == "5Kb"]
reshaped_df = df.melt(id_vars=["Resolution"], var_name="Algorithm", value_name="Value")
summary = reshaped_df.groupby(['Resolution', 'Algorithm'])['Value'].agg(['min', 'median', 'max']).reset_index()
# Rename the columns for better clarity
summary.columns = ['Resolution', 'Algorithm', 'Min Value', 'Median Value', 'Max Value']
# Save the summary to a CSV file
summary.to_csv('ch12lx_adj_r_statistics.csv', index=False)
...
plt.xlabel(f"Algorithm")
plt.ylabel(f"TADadjR$^2$")
plt.savefig("ch12lx_adj_square_boxplot.png", dpi=300, bbox_inches="tight")
...
for resolution in smooth_df["Resolution"].unique():
    ...
    plt.savefig(f"ch12lx_{resolution}_adj_r_lineplots.png", dpi=300, bbox_inches="tight")