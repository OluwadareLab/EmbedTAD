...
def find_overlaps(dataset1, dataset2):
    ...
    for _, row1 in dataset1.iterrows():
        overlap = dataset2[(dataset2["start"] == row1["start"])
                           & (dataset2["end"] == row1["end"])]
...
def main():
    ...
    plt.title("Overlap")
    plt.savefig("/home/hc0783.unt.ad.unt.edu/workspace/codebase/EmbedTAD/analysis/mus_diagram.png",
...
def test():
    ...
    overlaps_1_2 = find_overlaps(dataset1, dataset2)
    print(f"Naïve ∩ Th17: {len(overlaps_1_2)}")
    overlaps_1_3 = find_overlaps(dataset1, dataset3)
    print(f"Naïve ∩ Th1: {len(overlaps_1_3)}")
    overlaps_2_3 = find_overlaps(dataset2, dataset3)
    ...
    shared_12 = len(overlaps_1_2 - overlaps_1_3 - overlaps_2_3)
    print(f"Naïve ∩ Th17: {shared_12}")
    shared_13 = len(overlaps_1_3 - overlaps_1_2 - overlaps_2_3)
    print(f"Naïve ∩ Th1: {shared_13}")
    shared_23 = len(overlaps_2_3 - overlaps_1_2 - overlaps_1_3)
    print(f"Th17 ∩ Th1: {shared_23}")

    shared_all = len(overlaps_1_2 & overlaps_1_3 & overlaps_2_3)
    print(f"Naïve ∩ Th17 ∩ Th1: {shared_all}")
    ...
    # Set labels for sets
    ax.text(0.15, 0.12, "Naïve", fontsize=14, color="#000000")
    ax.text(0.8, 0.05, "Th17", fontsize=14, color="#000000")
    ax.text(0.75, 0.95, "Th1", fontsize=14, color="#000000")

    # Position subset counts in appropriate locations
    ax.text(0.16, 0.40, str(only_1), fontsize=12,
            color="#000000")  # Only Naive
    ax.text(0.70, 0.25, str(only_2), fontsize=12, color="#000000")  # Only Th17
    ax.text(0.48, 0.85, str(only_3), fontsize=12, color="#000000")   # Only Th1

    ax.text(0.41, 0.30, str(shared_12), fontsize=12,
            color="#000000")  # Naive ∩ Th17
    ax.text(0.32, 0.65, str(shared_13), fontsize=12,
            color="#000000")  # Naive ∩ Th1
    ax.text(0.64, 0.58, str(shared_23), fontsize=12,
            color="#000000")  # Th17 ∩ Th1

    ax.text(0.46, 0.49, str(shared_all), fontsize=14, color="#000000",
            fontweight="bold")  # Naive ∩ Th17 ∩ Th1 (center)
    ...
    plt.savefig("/home/hc0783.unt.ad.unt.edu/workspace/codebase/EmbedTAD/analysis/mus_overlap_venn_diagram.png",
                dpi=600, bbox_inches="tight")
